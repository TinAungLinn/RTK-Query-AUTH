import Cookies from "js-cookie";
import React from "react";
import { Link, useParams } from "react-router-dom";
import { useGetContactQuery } from "../redux/api/contactApi";

const UserInfo = () => {
  const { id } = useParams();
  const token = Cookies.get("token");

  const { data } = useGetContactQuery(token, id);

  const info = data?.contacts?.data[0];

  console.log(info);

  return (
    <div className=" flex justify-center items-center h-screen">
      <div className=" flex flex-col gap-3 shadow-lg w-52 p-7">
        <img
          width={"150px"}
          src={
            info?.photo === null
              ? "data:image/png;base64,"
              : info?.photo
          }
          alt=""
        />
        <h1 className=" text-gray-500">{info?.name}</h1>
        <h1 className=" text-gray-500">{info?.phone}</h1>
        <h1 className=" text-gray-500">{info?.address}</h1>
        <Link to={"/"}>
        <button className=" py-1 px-5 text-white rounded bg-gray-600 hover:bg-gray-500">
          Back
        </button>
        </Link>
      </div>
    </div>
  );
};

export default UserInfo;
